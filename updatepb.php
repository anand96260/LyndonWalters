...
if (isset($_POST['checkbox']) && is_array($_POST['checkbox']) ) {
	...
	foreach($array as $key=>$value){
		...
		$query="UPDATE  `lyndon_walter`.`pbooking`
		SET  `auth` =  '{$value}'
		WHERE  `pbooking`.`id` =$key";
		//echo "$query";
		mysql_query($query);
		
		if($value == 1){
		$email="SELECT `email`
				FROM `lyndon_walter`.`pbooking`
				WHERE `pbooking`.`id` =$key";
	...
	$name=$Email['username'];
	$emailsubject= "Your Provisional Bookings Accepted - Lyndonwalters.com";
  ...
  <tr>
    <td colspan="2"><div align="center"><strong>Provisional Bookings Accepted</strong></div></td>
  </tr>
  <tr>
    <td colspan="2">Dear Sir/Madam</td>
  </tr>
  <tr>
    <td colspan="2">Your Provisional Bookings Accepted.</td>
  </tr>
  <tr>
    <td colspan="2">Login Details:</td>
  </tr>
  <tr>
    <td colspan="2"><p>UserName:'.$email_id.' </p>
    <p>Password: Your Telephone Number.</p></td>
  </tr>
  <tr>
...
mysql_query("DELETE FROM `pbooking`
			 WHERE `auth` = 2");
} 
}
redirect_to("authpb.php");
?>