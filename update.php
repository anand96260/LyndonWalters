...
if (isset($_POST['checkbox']) && is_array($_POST['checkbox']) ) {
	...
	foreach($array as $key=>$value){
		...
		$query="UPDATE  `lyndon_walter`.`testimornial`
		SET  `auth` =  '{$value}'
		WHERE  `testimornial`.`id` =$key";
		//echo "$query";
		mysql_query($query);
	if($value == 1){
		$email="SELECT `email`
	...
	//$name=$Email['username'];
	$emailsubject= "Your Testimonial Accepted - Lyndonwalters.com";
  ...
  <tr>
    <td colspan="2">Your testimonial Accepted.</td>
  </tr>
  <tr>
    <td colspan="2">Thanks for your Testimonials.</td>
  </tr>
  <tr>
...
mysql_query("DELETE FROM `testimornial`
			 WHERE `auth` = 2");
} 
} 

redirect_to("authtestimonials.php");
?>
